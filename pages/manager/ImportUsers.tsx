...
import { BackButton } from '../../components/ui/BackButton';import React, { useState } from 'react';
import Papa from 'papaparse';
import { supabase } from '../../supabaseClient';
import { Card } from '../../components/ui/Card';
import { Button } from '../../components/ui/Button';
...
const CheckCircleIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5 mr-2 text-green-500"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
);
...
export const ImportUsers: React.FC = () => {
    ...
    const handleImport = () => {
        ...
        Papa.parse(file, {
            ...
            complete: async (results) => {
                ...
                const profilesToUpsert: TablesInsert<'profiles'>[] = parsedUsers
                ...
                if (error) {
                    ...
                    setMessage({ type: 'success', text: `${profilesToUpsert.length} users have been successfully imported or updated.` });
                }
                setLoading(false);
            },
            error: (error) => {
    ...
    return (
        <div className="container mx-auto p-4 sm:p-6 lg:p-8 max-w-2xl">
            ...
            <Card>
                <h1 className="text-3xl font-bold mb-2">Import Users from CSV</h1>
                <p className="text-gray-500 dark:text-gray-400 mb-6">Upload a CSV file with `name` and `email` columns to add or update users.</p>

                <div className="space-y-4">
                    <div>
                        <label htmlFor="csv-upload" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                            Select CSV File
                        </label>
                        <input
                            id="csv-upload"
                ...
                {message && (
                    <div className={`mt-6 p-4 rounded-md flex items-center text-sm ${message.type === 'success' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200' : 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-200'}`}>
                        {message.type === 'success' ? <CheckCircleIcon /> : <AlertTriangleIcon />}
                        <span>{message.text}</span>
                    </div>
                )}
